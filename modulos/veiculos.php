	...
	case 'incluir':
		if(isset($_POST['cadastrar'])):
			...
			elseif($_GET['q'] == 'loja'):
				...
				printMsg('Tipo de dono não definido.','erro');		
			endif;	
			$veiculo = new objVeiculos(array(
				"dono_id"		=> $dono_id,
				"pertencente" 	=> $_POST['pertencente'],
				"nome" 			=> $_POST['nome'],
				"preco"			=> $_POST['preco'],
				"cor"			=> $_POST['cor'],
			...
			elseif($_GET['q'] == 'loja'):
				if($veiculo->getValor('img_1')):
					criar_diretorio_veiculos($_POST['dono_nome'], $veiculo->getValor('img_1'));
					upload_mestre($veiculo->getValor('nome'),$_FILES['img_1'],$_POST['dono_nome'].'/'.$veiculo->getValor('img_1'),$veiculo->getValor('img_1'),'img_lojas',500);
					upload_mestre($veiculo->getValor('nome'),$_FILES['img_2'],$_POST['dono_nome'].'/'.$veiculo->getValor('img_1'),$veiculo->getValor('img_2'),'img_lojas',500);
					upload_mestre($veiculo->getValor('nome'),$_FILES['img_3'],$_POST['dono_nome'].'/'.$veiculo->getValor('img_1'),$veiculo->getValor('img_3'),'img_lojas',500);
					upload_mestre($veiculo->getValor('nome'),$_FILES['img_4'],$_POST['dono_nome'].'/'.$veiculo->getValor('img_1'),$veiculo->getValor('img_4'),'img_lojas',500);
				else:	
					printMsg('Pasta não pode ser criada, pois o nome não foi definido','erro');
				endif;
			else:
				echo printMsg('Não foi possivel definir a quem o veículo pertence.','erro'); 			
			endif;
			if($veiculo->linhas_afetadas == 1):
				printMsg('Dados inserido com sucesso <a href="'.ADMURL.'?m=veiculos&t=listar&q='.$_GET['q'].'">Exibir cadastros</a>');
				unset($_POST);
			endif;			
		endif;	
	?>
			...
			<form class="userForm" method="post" action="" enctype="multipart/form-data">
				...
				<?php 	
					if($_GET['q'] == 'usu'):
						...
						$usuarios->extras_select = " WHERE tipo= 'dono de carro'";
						$usuarios->seleciona_tudo($usuarios);
						
				?>
						<li><label for="dono_nome">Dono do anuncio:</label> <select name="dono_nome" autofocus="autofocus">
				<?php 	
						echo '<option> </option>';
				...
				?>
						<li><label for="dono_nome">Loja:</label> <select name="dono_nome" autofocus="autofocus">
				<?php 
						echo '<option> </option>';
				...
				?>	
					...
					<li><label for="pertencente">Pertence:</label> <select name="pertencente">
				<?php if($_GET['q'] == 'usu'):?>		
				...
				<?php endif;?>		
					...
					<li class="center"><input type="button"
						onclick="location.href='?m=veiculos&t=listar&q=<?php echo $_GET['q'] ?>'" value="Cancelar" /> <input
				 ...
				 <thead>
				  ...
				  <tbody>
					<?php 
						...
						if($_GET['q'] == 'usu'):
							$veiculo->extras_select = " WHERE pertencente='exclusivo'";
							$veiculo->seleciona_tudo($veiculo);
						elseif($_GET['q'] == 'loja'):
							$veiculo->extras_select = " WHERE pertencente='loja'";
							$veiculo->seleciona_tudo($veiculo);
						endif;
						...
						while($resp = $veiculo->retorna_dados()):
							$id = NULL;
							if($_GET['q'] == 'usu'):
								$usu_id = new objVeiculos();
								$usu_id->retornarUsuarioNome($resp->dono_id);
								$id_e = $usu_id->retorna_dados();
								$id = $id_e->nome;
							elseif($_GET['q'] == 'loja'):
								$loja_id = new objLojas();
								$loja_id->retornarLojaNome($resp->dono_id);
								$id_l = $loja_id->retorna_dados();
								$id = $id_l->nome;
							endif;	
							echo '<tr>'; 
								printf('<td class="center">%s</td>',$id);
								...
								<td class="center">
									<div>
										<a href="?m=veiculos&t=incluir&q=%s" title="Novo cadastro"><img src="img/plus.png" alt="Novo cadastro" /></a>
										<a href="?m=veiculos&t=editar&q=%s&id=%s" title="Editar"><img src="img/edit.png" alt="Editar" /></a> 
										<a href="?m=veiculos&t=excluir&q=%s&id=%s" title="Excluir"><img src="img/cancel.png" alt="Excluir" /></a>
									</div>
								</td>',$_GET['q'],$_GET['q'],$resp->id,$_GET['q'],$resp->id);
							echo '</tr>';
			...
			if(isAdmin() == TRUE || $sessao->getVar('id_user') == $_GET['id']):
				if(isset($_GET['id'])):
					...
					if(isset($_POST['editar'])):
						...
						$veiculo_bd = new objVeiculos(array(
							"dono_id"		=> $dono_id,
							"pertencente" 	=> $_POST['pertencente'],
							"nome" 			=> $_POST['nome'],
							"preco"			=> $_POST['preco'],
						...
						if($veiculo_bd->linhas_afetadas == 1):
							printMsg('Dados alterados com sucesso. <a href="?m=veiculos&t=listar&q='.$_GET['q'].'">Exibir cadastros</a>');
							unset($_POST);
						else:
							printMsg('Nenhum dado foi alterado. <a href="?m=veiculos&t=listar&q='.$_GET['q'].'">Exibir cadastros</a>','alerta');	
						endif;
						
					endif;
					
					$veiculo_dono = new objVeiculos();
					...
					if($atualizar_file == TRUE):
						...
						elseif($B_IMG_1 && $_GET['q'] == 'loja'):
							$dir = 'img_lojas/'.$l->getValor('nome').'/'.$B_IMG_1;
							apaga_diretorio($dir);
							criar_diretorio_veiculos($l->getValor('nome'), $veiculo_bd->getValor('img_1'));
							upload_mestre($l->getValor('nome'),$_FILES['img_1'],$l->getValor('nome').'/'.$veiculo_bd->getValor('img_1'),$veiculo_bd->getValor('img_1'),'img_lojas',500);
							upload_mestre($l->getValor('nome'),$_FILES['img_2'],$l->getValor('nome').'/'.$veiculo_bd->getValor('img_1'),$veiculo_bd->getValor('img_2'),'img_lojas',500);
							upload_mestre($l->getValor('nome'),$_FILES['img_3'],$l->getValor('nome').'/'.$veiculo_bd->getValor('img_1'),$veiculo_bd->getValor('img_3'),'img_lojas',500);
							upload_mestre($l->getValor('nome'),$_FILES['img_4'],$l->getValor('nome').'/'.$veiculo_bd->getValor('img_1'),$veiculo_bd->getValor('img_4'),'img_lojas',500);
						else:
			...
			<form class="userForm" method="post" action="" enctype="multipart/form-data">
				...
				<?php 
					...
					<?php 
						...
						endwhile;
					?>
				<?php elseif($_GET['q'] == 'loja'):?>
				<?php 
						$loja = new objLojas();
						$loja->retornarLojas(); 
						
						if($veiculo_dono_r):
							$id = $veiculo_dono_r->dono_id;
				...
				<?php endif;?>		
					...
					<li><label for="pertencente">Pertence:</label> <select name="pertencente">
						<option><?php if($veiculo_dono_r) echo $veiculo_dono_r->pertencente ?></option>
						<option>exclusivo</option>
						<option>loja</option>
					</select>
					</li>
					...
					<li class="center">
					<input type="button" onclick="location.href='?m=veiculos&t=listar&q=<?php echo $_GET['q']?>'" value="Cancelar" /> 
					<input type="submit" name="editar" value="Editar dados" /></li>
				</ul>
				</fieldset>
			</form>
			...
			if(isAdmin() == TRUE):
				if(isset($_GET['id'])):
					...
					if(isset($_POST['excluir'])):
						...
						$deletar_pasta = TRUE;
						if($veiculo_bd->linhas_afetadas == 1):
							printMsg('Registro excluido com sucesso. <a href="?m=veiculos&t=listar&q='.$_GET['q'].'">Exibir cadastros</a>');
							unset($_POST);
						else:
							printMsg('Nenhum registro foi excluido. <a href="?m=veiculos&t=listar&q='.$_GET['q'].'">Exibir cadastros</a>','alerta');	
						endif;
					endif;
					...
					printMsg('Veículo não definido, <a href="m=veiculos_usu&t=listar&q='.$_GET['q'].'">Escolha um veículo para excluir</a>','erro');
				endif;
				?>
				...
				?>
					...
					<li><label for="pertencente">Pertence:</label> <select name="pertencente" disabled="disabled">
						<option><?php if($veiculo_resp) echo $veiculo_resp->pertencente ?></option>
					</select>
					</li>
					...
					<li class="center">
					<input type="button" onclick="location.href='?m=veiculos&t=listar&q=<?php echo $_GET['q']?>'" value="Cancelar" /> 
					<input type="submit" name="excluir" value="Excluir dados" /></li>
				</ul>
				</fieldset>
			</form>
